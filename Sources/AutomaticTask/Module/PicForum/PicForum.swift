...
class PicForum: ATBaseTask {
    ...
    private func forum(host: String? = nil, isCheckHost: Bool = false) {
        ...
        // 提取formhash
        if formhash.isEmpty {
            let regex = try! Regex("formhash=(\\w+)")
            if let formhash = regex.firstGroup(in: data.html), formhash.count > 3 {
    ...
    private func reply(comment: String, fid: Int, tid: String, name: String) -> Bool {
        ...
        if data.findSuccess(txt: "非常感謝，回復發佈成功") {
            ...
            if !user.reloadMoney() {
                ...
                log.print(text: "评论达到每日上限。不再发表评论。", type: .Warn)
    ...
    private func journal() {
        ...
        if data.findSuccess(txt: title) {
            ...
            if !user.reloadMoney() {
                // 如果发表后，金币数不增加，就不再发表
                log.print(text: "发表日志达到每日上限。", type: .Warn)
